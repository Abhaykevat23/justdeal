...
try {
    ...
    $fetch_post = "SELECT * FROM `lawyer` WHERE id=:id";
    ...
    if ($fetch_stmt->rowCount() > 0) :
        ...
        $lawyername = isset($data->lawyername) ? $data->lawyername : $row['lawyername'];
        $sunday = isset($data->sunday) ? $data->sunday : $row['sunday'];
       ...
       $update_query = "UPDATE `lawyer` SET price = :price,image=:image,lawyername=:lawyername,sunday=:sunday,mobile=:mobile,
        ...
        $update_stmt->bindValue(':lawyername', htmlspecialchars(strip_tags($lawyername)), PDO::PARAM_STR);
        $update_stmt->bindValue(':sunday', htmlspecialchars(strip_tags($sunday)), PDO::PARAM_STR);
